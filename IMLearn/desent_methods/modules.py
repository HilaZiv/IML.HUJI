...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        return -np.sum((y * (X @ self.weights)) - np.log(1 + np.exp(X @ self.weights))) / X.shape[0]
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        return -((X.T @ y) - (X.T @ (np.exp(X @ self.weights) * (1 / (1 + np.exp(X @ self.weights)))))) / X.shape[0]
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        if self.include_intercept_:
            self.regularization_module_.weights = self.weights[1:]
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        if self.include_intercept_:
            self.regularization_module_.weights = self.weights[1:]
        fidelity_jac = self.fidelity_module_.compute_jacobian(**kwargs)
        regularization_jac = self.regularization_module_.compute_jacobian(**kwargs)
        if self.include_intercept_:
            regularization_jac = np.insert(regularization_jac, 0, 0)
        return fidelity_jac + (self.lam_ * regularization_jac)
